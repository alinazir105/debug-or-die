...
export const BackgroundBeams = React.memo(({ className }) => {
  const resetTimer = () => {
    ...
    localStorage.setItem("system_failure_timer_end", newEndTime.toString());
  };
  const paths = [
  ...
  return (
    ...
    >
      <h1 className="absolute top-[35%] left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-red-500 text-7xl font-bold z-10">
        Debug Or Die
      </h1>
      <Link
        to="/homepage"
        onClick={resetTimer}
        className="absolute top-[52%] left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white text-neutral-900 rounded-lg px-8 py-4 text-2xl font-bold z-10 hover:bg-red-500 hover:text-white transition-all duration-200"
      >
        Start Game
      </Link>
      <p className="animate-pulse absolute top-[65%] left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-3/4 text-center text-red-500 text-lg font-mono uppercase tracking-wider z-10">
        ⚠ WARNING: Pressing Start will begin the competition! 90-minute
        countdown starts immediately.
      </p>
      <svg
      ...
      >
        ...
        {paths.map((path, index) => (
          <motion.path
            ...
            strokeWidth="0.5"
            initial={{
              opacity: 0.4, // Set initial opacity to match the desired visible state
            }}
          ></motion.path>
        ))}
        <defs>
          {paths.map((path, index) => (
            <motion.linearGradient
              ...
              initial={{
                x1: "0%",
                x2: "0%",
                y1: "100%", // Start from a visible position
                y2: "100%",
              }}
              animate={{
                x1: ["0%", "100%"],
              ...
              transition={{
                ...
                ease: "easeInOut",
                repeat: Infinity,
                delay: 0, // Remove any initial delay
              }}
            >
