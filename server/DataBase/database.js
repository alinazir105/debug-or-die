...
const getQueryByIdAndUpdate = async (queryId, options) => {
  return await Query.findOneAndUpdate({ _id: queryId }, options, { new: true });
};
...
const findOneAndUpdate = async (userId, quesName, buggyCode) => {
  const user = await User.findById(userId);
  if (!user) throw new Error("User not found");

  user.submissions.set(quesName, buggyCode); // Update using `set()` method
...
module.exports = {
  ...
  Query: {
    ...
    deleteQueryById,
    getQueryByIdAndUpdate,
  },
  User: {
    createNewUser,
    getUserById,
    findOneUser,
    addSolvedQuestionToUser,
    incrTotalSubmInUser,
    findOneAndUpdate,
  },
